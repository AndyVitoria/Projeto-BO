...
def end2cord(entrada, saida, boletim_dir):
    entrada += '/'
    saida += '/'
    ...
    # Verifica se existe uma lista com endereços convertidos
    try:
        lista_enderecos = Arquivo.json('enderecos_coordenadas.json')
    except:
        lista_enderecos = dict()
...
def main():
    end2cord(entrada='BOLETIM', saida='MAPEADOS', boletim_dir='B/B01/boletim.json')
